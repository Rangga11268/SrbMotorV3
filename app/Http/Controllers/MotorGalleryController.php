...
class MotorGalleryController extends Controller
{
    ...
    public function index(Request $request): \Inertia\Response|JsonResponse
    {
        ...
        $filterOptions = $this->motorRepository->getFilterOptions(
            $request->get("search"),
        );
    ...
    public function processCashOrder(
        ...
        if ($request->booking_fee && $request->booking_fee >= $motor->price) {
            return back()
                ->withErrors([
                    "booking_fee" =>
                        "Booking fee tidak boleh melebihi atau sama dengan harga motor.",
                ])
                ->withInput();
        }
        ...
        if (!$motor->tersedia) {
            return back()
                ->withErrors([
                    "motor" => "Motor ini sedang tidak tersedia untuk dipesan.",
                ])
                ->withInput();
        }
        ...
        if ($existingOrder) {
            return back()
                ->withErrors([
                    "motor" =>
                        "Anda sudah memiliki pesanan aktif untuk motor ini.",
                ])
                ->withInput();
        }
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::error(
                "WA Notification Error: " . $e->getMessage(),
            );
        }

        return redirect()
            ->route("motors.order.confirmation", [
                "transaction" => $transaction->id,
            ])
            ->with(
                "success",
                "Pesanan tunai Anda telah dibuat. Silakan lanjutkan ke halaman konfirmasi.",
            );
    }
    ...
    {
        return \Inertia\Inertia::render(
            "Motors/CreditOrderForm",
            compact("motor"),
        );
    }

    public function processCreditOrder(
        Request $request,
        Motor $motor,
    ): RedirectResponse {
        $request->validate([
        ...
        if ($request->dp_amount < $minDownPayment) {
            return back()
                ->withErrors([
                    "dp_amount" =>
                        "Uang muka minimum untuk motor ini adalah Rp " .
                        number_format($minDownPayment, 0, ",", ".") .
                        ".",
                ])
                ->withInput();
        }
        ...
        if ($request->dp_amount >= $motor->price) {
            return back()
                ->withErrors([
                    "dp_amount" =>
                        "Uang muka tidak boleh melebihi atau sama dengan harga motor.",
                ])
                ->withInput();
        }
        ...
        return redirect()
            ->route("motors.upload-credit-documents", [
                "transaction" => $transaction->id,
            ])
            ->with(
                "success",
                "Pengajuan kredit berhasil dibuat. Silakan lengkapi dokumen untuk melanjutkan proses.",
            );
    }
    ...
    public function showOrderConfirmation($transactionId): \Inertia\Response
    {
        ...
        // Get branch data if branch_code exists
        $branch = null;
        if ($transaction->branch_code) {
            $branchService = app(\App\Services\BranchService::class);
            $branch = $branchService->getBranchByCode(
    ...
    public function showUploadCreditDocuments(
        $transactionId,
    ): View|\Inertia\Response {
        $transaction = Transaction::with([
            "motor",
            "creditDetail",
            "creditDetail.documents",
    ...
    {
        $transaction = Transaction::with(["creditDetail"])->findOrFail(
            $transactionId,
        );
        ...
        $request->validate(
            ...
            [
                ...
                "documents.KTP.*.mimes" =>
                    "File KTP harus berupa file gambar (jpg, jpeg, png) atau PDF",
                "documents.KTP.*.max" =>
                    "Ukuran file KTP tidak boleh lebih dari 2MB",
                ...
                "documents.KK.*.mimes" =>
                    "File KK harus berupa file gambar (jpg, jpeg, png) atau PDF",
                "documents.KK.*.max" =>
                    "Ukuran file KK tidak boleh lebih dari 2MB",
                "documents.SLIP_GAJI.required" =>
                    "File slip gaji wajib diunggah",
                "documents.SLIP_GAJI.min" =>
                    "Minimal 1 file slip gaji harus diunggah",
                "documents.SLIP_GAJI.*.file" =>
                    "File slip gaji harus berupa file yang valid",
                "documents.SLIP_GAJI.*.mimes" =>
                    "File slip gaji harus berupa file gambar (jpg, jpeg, png) atau PDF",
                "documents.SLIP_GAJI.*.max" =>
                    "Ukuran file slip gaji tidak boleh lebih dari 2MB",
                "documents.LAINNYA.*.file" =>
                    "File tambahan harus berupa file yang valid",
                "documents.LAINNYA.*.mimes" =>
                    "File tambahan harus berupa file gambar (jpg, jpeg, png) atau PDF",
                "documents.LAINNYA.*.max" =>
                    "Ukuran file tambahan tidak boleh lebih dari 2MB",
            ],
        );
        ...
        foreach ($request->file("documents", []) as $docType => $files) {
            if (is_array($files)) {
                foreach ($files as $file) {
                    if ($file) {
                        $path = $file->store(
                            "credit-documents/" . $transaction->id,
                            "public",
                        );

                        Document::create([
                            "credit_detail_id" =>
                                $transaction->creditDetail->id,
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::error(
                "WA Notification Error (upload docs): " . $e->getMessage(),
            );
        }
        ...
        return redirect()
            ...
            ->with(
                "success",
                "Dokumen berhasil diunggah. Pengajuan kredit Anda sedang dalam proses review.",
            );
    }

    public function showDocumentManagement(
        $transactionId,
    ): View|\Inertia\Response {
        $transaction = Transaction::with([
            "motor",
    ...
    public function updateDocuments(Request $request, $transactionId)
    {
        ...
        if (!$transaction->creditDetail) {
            return redirect()
                ...
                ->withErrors([
                    "error" =>
                        "Transaksi tidak memiliki detail kredit yang valid.",
                ]);
        }
        ...
        if (in_array($transaction->creditDetail->status, $lockedStatuses)) {
            return redirect()
                ...
                ->withErrors([
                    "error" =>
                        "Pengajuan Anda sedang diproses. Dokumen tidak dapat diubah saat ini.",
                ]);
        }
        ...
        if ($documentsProcessed) {
            ...
            return redirect()
                ->route("credit-status.show", [
                    "transaction" => $transaction->id,
                ])
                ->with(
                    "success",
                    "Dokumen berhasil diperbarui. Pengajuan kredit Anda sedang dalam proses review.",
                );
        } else {
            return redirect()
                ->route("credit-status.show", [
    ...
    public function cancelOrder(
        ...
        $result = $creditService->cancelByCustomer(
            $transaction,
        ...
        try {
            ...
            $userMsg =
                "Pesanan motor *{$motor->name}* (Order ID: #{$transaction->id}) telah dibatalkan.\n\nTanggal pembatalan: " .
            ...
            if ($adminPhone) {
                $statusLabel =
                    $transaction->transaction_type === "CASH"
                        ...
                        : "Kredit";
                $adminMsg =
                    "*[ADMIN] Order {$statusLabel} Dibatalkan oleh Pelanggan*\n\nPelanggan: {$transaction->name}\nUnit: {$motor->name}\nAlasan: " .
            ...
            \Illuminate\Support\Facades\Log::error(
                "WA Notification Error on cancellation: " . $e->getMessage(),
            );
        }
    ...
    public function rejectDocument(
        ...
        try {
            ...
            return back()->with(
                "success",
                "Dokumen telah ditolak dan notifikasi telah dikirim ke customer.",
            );
        } catch (\Exception $e) {
            ...
            return back()->with(
                "error",
                "Terjadi kesalahan saat menolak dokumen.",
            );
        }
    }
    ...
    public function scheduleSurvey(Request $request, $creditDetailId)
    {
        ...
        try {
            ...
            return back()->with(
                "success",
                "Jadwal survey telah dibuat dan notifikasi telah dikirim ke customer.",
            );
        } catch (\Exception $e) {
            ...
            return back()->with(
                "error",
                "Terjadi kesalahan saat menjadwalkan survey.",
            );
        }
    }
    ...
    public function confirmSurveyCompletion($surveyScheduleId)
    {
        ...
        try {
            $schedule = \App\Models\SurveySchedule::findOrFail(
                $surveyScheduleId,
            );
            $schedule->complete();
            ...
            return back()->with(
                "success",
                "Survey telah ditandai sebagai selesai.",
            );
        } catch (\Exception $e) {
            ...
            return back()->with(
                "error",
                "Terjadi kesalahan saat mengkonfirmasi survey.",
            );
        }
    }
    ...
    public function showTransaction(Transaction $transaction)
    {
        ...
        if ($transaction->creditDetail) {
            $transactionData[
                "creditDetail"
            ] = $transaction->creditDetail->toArray();
            $transactionData["creditDetail"][
                "documents"
            ] = $transaction->creditDetail->documents->toArray();
            $transactionData["creditDetail"][
                "surveySchedules"
            ] = $transaction->creditDetail->surveySchedules->toArray();
        }
     ...
     */
    public function confirmSurveyAttendance(
        Request $request,
        \App\Models\SurveySchedule $surveySchedule,
    ) {
        ...
        try {
            ...
            $surveyor_msg =
                "Pelanggan *{$transaction->user->name}* telah mengkonfirmasi kehadiran survei pada *" .
                $surveySchedule->scheduled_date->format("d-m-Y H:i") .
                "* di " .
            ...
            return back()->with(
                "success",
                "Kehadiran survei telah dikonfirmasi. Surveyor akan segera menghubungi Anda.",
            );
        } catch (\Exception $e) {
            ...
            return back()->withErrors([
                "survey" => "Terjadi kesalahan saat mengkonfirmasi kehadiran.",
            ]);
        }
    }
     ...
     */
    public function requestSurveyReschedule(
        Request $request,
        \App\Models\SurveySchedule $surveySchedule,
    ) {
        // Authorize - customer owns this survey
        ...
        if (!in_array($surveySchedule->status, ["pending", "confirmed"])) {
            return back()->withErrors([
                "survey" => "Survei ini tidak dapat dijadwal ulang.",
            ]);
        }
        ...
        try {
            ...
            $surveyor_msg =
                "Pelanggan *{$transaction->user->name}* telah mengajukan jadwal survei baru.\n\n" .
            ...
            return back()->with(
                "success",
                "Jadwal survei telah diperbarui. Admin akan mengkonfirmasi jadwal baru Anda.",
            );
        } catch (\Exception $e) {
            ...
            return back()->withErrors([
                "survey" => "Terjadi kesalahan saat memperbarui jadwal survei.",
            ]);
        }
    }
