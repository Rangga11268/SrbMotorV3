            ...
            <section className="bg-black pt-28 pb-20 relative overflow-hidden text-white border-b border-gray-800">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col md:flex-row items-center gap-12">
                    ...
                    <div className="w-full md:w-1/2 space-y-6">
                        <div className="mb-2">
                            <Link href={route("motors.index")} className="inline-flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest hover:text-white transition-colors group">
                        ...
                        <nav className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-[#1c69d4] mb-4">
                            ...
                            <Link href={route("motors.index")} className="hover:text-white transition-colors">KATALOG</Link>
                            <ChevronRight className="w-3 h-3 text-gray-600" />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                        ...
                        <div className="lg:col-span-8 space-y-16">
                            ...
                            <div className="space-y-8">
                                <h2 className="text-3xl font-black uppercase tracking-tighter text-black flex items-center gap-3">
                                    <span className="w-8 h-1 bg-[#1c69d4]"></span> SPESIFIKASI TEKNIS
                                </h2>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-gray-200 border border-gray-200 rounded-none">
                                    {[
                                        ...
                                        { label: "WARNA", value: Array.isArray(motor.colors) && motor.colors.length > 0 ? motor.colors.join(", ") : "BERAGAM" },
                                        { label: "TRANSMISI", value: motor.type?.toLowerCase().includes("matic") ? "MATIC" : "MANUAL" },
                                        { label: "STATUS", value: motor.tersedia ? "TERSEDIA" : "TERJUAL" }
                                    ].map((spec, i) => (
                                        <div key={i} className="bg-white p-6 flex flex-col justify-center items-start group hover:bg-gray-50 transition-colors">
                            ...
                            <div className="space-y-8">
                                <h2 className="text-3xl font-black uppercase tracking-tighter text-black flex items-center gap-3">
                                    <span className="w-8 h-1 bg-[#1c69d4]"></span> KETERANGAN
                                </h2>
                                <div
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {[
                                    { title: "GARANSI MESIN", desc: "1 TAHUN COVERAGE" },
                                    { title: "LEGALITAS JELAS", desc: "STNK & BPKB READY" },
                                    { title: "FULL SERVICE", desc: "GRATIS SERVIS 1X" }
                                ].map((benefit, i) => (
                                    <div key={i} className="border border-gray-200 p-8 rounded-none hover:border-[#1c69d4] transition-colors bg-gray-50">
                                        <h4 className="text-lg font-black text-black mb-2 uppercase tracking-tight">{benefit.title}</h4>
                                        <p className="text-[11px] text-gray-500 font-bold uppercase tracking-widest">{benefit.desc}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <aside className="lg:col-span-4 space-y-8">
                            ...
                            <div className="bg-black text-white p-8 rounded-none">
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">HARGA KENDARAAN</p>
                                ...
                                <div className="space-y-3">
                                    {motor.tersedia ? (
                                        ...
                                        <button disabled className="w-full bg-gray-800 text-gray-500 font-bold text-[13px] uppercase tracking-[0.15em] py-4 rounded-none cursor-not-allowed">
                                            UNIT TERJUAL
                                        </button>
                                    )}
                                    <button 
                        ...
                        <div className="mt-24 pt-16 border-t border-gray-200">
                            <div className="flex items-end justify-between mb-12">
                                <h2 className="text-4xl font-black uppercase tracking-tighter text-black flex items-center gap-3">
                                    <span className="w-8 h-1 bg-black"></span> UNIT TERKAIT
                                </h2>
                                <Link href="/motors" className="hidden md:flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-[#1c69d4] hover:text-black transition-colors">
                                ...
                                {relatedMotors.map((m) => (
                                    <Link key={m.id} href={route("motors.show", m.id)} className="bg-white group overflow-hidden flex flex-col">
                                        ...
                                        <div className="p-6 mt-auto">
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{m.brand}</p>
                                            <h4 className="text-lg font-black text-black uppercase tracking-tight mb-2 limit-1-line group-hover:text-[#1c69d4] transition-colors">{m.name}</h4>
                                            <p className="text-lg font-light text-gray-600">Rp {parseInt(m.price).toLocaleString("id-ID")}</p>
                                        </div>
                                    </Link>
                                ))}
                            </div>
